...
// Query implementations using test db
// These mirror the actual queries in lib/db/queries.ts to test the same SQL logic
...
async function getChatById({ id }: { id: string }) {
...
async function deleteChatById({ id }: { id: string }) {
...
async function deleteAllChatsByUserId({ userId }: { userId: string }) {
	...
	await db.delete(schema.vote).where(inArray(schema.vote.chatId, chatIds));
	await db.delete(schema.message).where(inArray(schema.message.chatId, chatIds));
	await db.delete(schema.stream).where(inArray(schema.stream.chatId, chatIds));
...
async function getChatsByUserId({
	...
	let filteredChats: (typeof schema.chat.$inferSelect)[] = [];
...
async function saveMessages({ messages }: { messages: DBMessage[] }) {
	return await db.insert(schema.message).values(messages);
}

async function getMessagesByChatId({ id }: { id: string }) {
	return await db
		.select()
		.from(schema.message)
...
async function getMessageById({ id }: { id: string }) {
...
async function voteMessage({
	...
	const [existingVote] = await db
		.select()
		.from(schema.vote)
		.where(eq(schema.vote.messageId, messageId));
...
async function getVotesByChatId({ id }: { id: string }) {
	return await db.select().from(schema.vote).where(eq(schema.vote.chatId, id));
}

async function getLatestChatForUser({ userId }: { userId: string }) {
...
async function getStreamIdsByChatId({ chatId }: { chatId: string }) {
...
async function getMessageCountByUserId({
	...
	const [stats] = await db
		...
		.where(
			and(
				eq(schema.chat.userId, id),
				gte(schema.message.createdAt, hoursAgo),
...
async function getWebhookLogByMessageSid({
	messageSid,
}: {
	messageSid: string;
}) {
	const [existing] = await db
		...
		.where(eq(schema.webhookLog.messageSid, messageSid))
		.limit(1);

	return existing ?? null;
}

async function createPendingWebhookLog(entry: {
	source: string;
	requestUrl: string;
	messageSid: string;
	fromNumber: string;
	...
	const [created] = await db
		...
		.onConflictDoNothing({
			target: schema.webhookLog.messageSid,
		})
...
describe("Webhook log queries", () => {
	...
	it("should save webhook log", async () => {
		...
		// Verify by checking all logs
		const logs = await db.select().from(schema.webhookLog);

		expect(logs.length).toBeGreaterThan(0);
		expect(logs[0].source).toBe("twilio");
	});
});
